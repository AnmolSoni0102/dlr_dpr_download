    ...
    try {
        ...
        if (type == "byCategory") {
            ...
            productivityDprData = await con.execute(
                `select DATE(dd.created_at) as dt, SUM(qty)*ccdip.item_price as expected_outcome,
            ...
            GROUP BY DATE(dd.created_at), dd.category_id ORDER BY DATE(dd.created_at) ASC`,
                [bookingId, category_id, month]
            );
            ...
            productivityDprData = await con.execute(
                `select dat.dt, SUM(dat.expected_outcome) as expected_outcome, SUM(dat.actual_outcome) as actual_outcome from (select DATE(dd.created_at) as dt, SUBSTRING(dd.dpr_item, 1, 5) as item_code, 
        ...
        console.log(
            "productivityData  ",
            productivityDlrData[0],
            productivityDprData[0]
        );
        return {
            productivityDlrData: productivityDlrData[0],
            productivityDprData: productivityDprData[0],
        };
    } catch (ex) {
        console.log(ex);
    }
...
const filterByWeekorMonth = async (dbData, filterBy) => {
    ...
    let { productivityDlrData, productivityDprData } = dbData,
        ...
        totalDpr = {},
        count = 0;
    const productivityDprDatainObj =
        convertToObjectWithDateKey(productivityDprData);
    ...
    for (let i = 0; i < productivityDlrData.length; i++) {
        ...
        let weekhash2 =
            filterBy == "weekly"
                ? weekOfTheMonthYear(new Date(moment(dlr.dt).format("YYYY-MM-DD")))
        ...
        const dprData =
            productivityDprDatainObj[
            new Date(moment(dlrData.dt).format("YYYY-MM-DD"))
            ];
        ...
        if (dprData) {
            dpr = {
                expected_outcome:
                    (dpr.expected_outcome ? dpr.expected_outcome : 0) +
                    dprData.expected_outcome,
                actual_outcome:
                    (dpr.actual_outcome ? dpr.actual_outcome : 0) +
                    dprData.actual_outcome,
            };
            totalDpr = {
                expected_outcome: dpr.expected_outcome,
                actual_outcome: dpr.actual_outcome,
            };
        }
    ...
    if (productivityDlrData.length == 0) {
    ...
    return {
        ...
        total: {
            totalDlr: { ...totalDlr },
            totalDpr: { ...totalDpr },
            averageDlr: { dlr: totalDlr.dlr / count },
            averageDpr: {
